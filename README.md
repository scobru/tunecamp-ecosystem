...
- **ZEN/Gun.js role in identity.** Gun.js/ZEN is used client-side for Zen SEA self-sovereign graph identity (`~pubKey/linked_instances`) via P2P relay nodes (e.g. `wss://delay.scobrudot.dev/zen` or community-hosted relays from `scobru/zen`), keeping core server dependencies lightweight while supporting cross-instance public profile/release aggregation. See `docs/tunecamp/FID-IDENTITY.md`.

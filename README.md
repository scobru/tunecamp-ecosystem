...
- **Instance Autonomy & Zen SEA Passports.** Instance auth remains local (username + password + JWT, per-instance). Cross-instance identity unification is achieved via **Zen SEA & Instance Passports** (`GET/POST /api/auth/zen/*`), where users bind local accounts to a global `scobru/zen` `~pubKey` on `tunecamp.org` via cryptographic proof of ownership, without central SSO.
- **ZEN/Gun.js role in identity.** Gun.js/ZEN is used client-side for Zen SEA self-sovereign graph identity (`~pubKey/linked_instances`) via relay `wss://delay.scobrudot.dev/zen`, keeping core server dependencies lightweight while supporting cross-instance public profile/release aggregation. See `docs/tunecamp/FID-IDENTITY.md`.

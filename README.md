...
- **Ephemeral P2P Audio Drop Protocol (Instance-Backed Swarm).**
  - **Rationale:** During live performances, the artist displays a QR code on stage generated by their TuneCamp instance. Audiences scanning the QR open a lightweight mobile PWA served by the instance. Instead of the instance proxying the heavy audio bandwidth to thousands of fans, it acts purely as a WebRTC signaling tracker (`/ws/drop`). The fans' phones form a P2P mesh network, seeding the live audio stream (originating from the artist's Sidecamp) to each other to drastically reduce server load.
  - **Stack:** Node.js WebSocket signaling (TuneCamp core), Browser WebRTC mesh logic (PWA), and Web Audio API for synced playback.
  - **Would touch:** TuneCamp core (new `/api/drop` endpoints and WebSocket tracker), TuneCamp frontend (mobile-first drop listener UI), and Sidecamp (broadcast trigger).

- **Air-Gapped Optical Audio & Data Transfer (Dynamic QR Code Stream).**
  - **Rationale:** Secure, offline, uni-directional transfer of small audio files (~2MB stems) or sensitive cryptographic keys (like FID Zen SEA Passports) out of air-gapped devices. Because webcams inevitably drop frames, it uses **Fountain Codes** (e.g., Luby Transform / RaptorQ). The sender loops an infinite stream of encoded QR "droplets"; the receiver simply scans until it captures ~105% of the chunks to perfectly reconstruct the file, requiring no back-channel communication.
  - **Stack:** Browser/Node (Lab App or Sidecamp integration), JavaScript/WASM Fountain Code algorithm, fast WASM-based QR scanner (`getUserMedia`), and Canvas API for 30/60 FPS generation.
  - **Would touch:** New standalone Lab App (`tunecamp-optical-transfer`) embedded in Sidecamp or TuneCamp core, allowing offline extractions of Zen keys or stems.

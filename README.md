...
- **Shared flagship instance (centrally hosted by the project, subdomain of tunecamp.org).** A generic, non-label-oriented TuneCamp deployment ("Bandcamp-alternative" positioning, not the curator/label workflow) — artist registers, publishes via ActivityPub, capped at ~30 tracks. Hosting is centralized (the project runs it, not the artist), unlike every other TuneCamp instance today. Highest-priority idea of the three logged here: lowest friction of any onboarding path (no deploy at all) and the direct brand vehicle for tunecamp.org, likely the best funnel toward full self-hosted adoption or the other two ideas below. The track cap is the deliberate mitigation for both scale concerns it raises: keeps per-artist storage and SQLite single-writer load bounded, and keeps DMCA/content-liability exposure (inherent to any platform hosting others' content, same as Bandcamp/DistroKid) manageable via a standard ToS + takedown policy rather than a novel problem.
- **Bandcamp importer.** Standalone tool (could be serverless/one-shot) that pulls an artist's existing Bandcamp discography metadata (tracklist, artwork, embedded page JSON) and pushes it into the TuneCamp network via the normal publish API — onto a shared instance or a static page (see below). Safe scope: metadata/artwork import only; original master audio still uploaded by the artist directly, since Bandcamp exposes no official bulk-export API for full-quality audio and preview streams are typically 128kbps. Lowest engineering cost of the ideas logged here — no new storage/network primitives, just an ETL bridge into an existing publish flow.
- **Faircamp-style static pages connected to the network.** Artist-facing static site generator (fork/wrap [Faircamp](https://simonrepp.com/faircamp/) rather than building from scratch) that emits a small manifest (NodeInfo-like JSON) alongside the generated static site so it can be indexed by the Community Directory / gossip crawler, without the site itself running any server logic or being a full ActivityPub actor. Matches the "serverless" goal cleanly: zero compute, zero storage-proxy problem (files served from whatever static host the artist picks), no P2P availability risk. Best low-risk first step for the wider "serverless TuneCamp-lite" concept — cheapest to build, proves whether "lightweight network member" pages get adoption, and the manifest format it defines can be reused by the Bandcamp importer's output and by a future shared-instance idea.

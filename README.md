...
- **Sample pack aggregator + Sound Designer persona (not yet built).** In-instance sample/sample-pack storage shipped in TuneCamp core (see below) — free, no credit/points economy, own CC0/CC-BY-style licensing separate from the artist revenue-split model. Still open: a standalone `tunecamp-samples` aggregator site (pure display, same shape as `tunecamp.org`'s catalog federation — lists samples pulled from `/api/samples` + `/api/sample-packs` across federated instances, links to each origin instance's download URL, no state of its own); the "Become a Sound Designer" onboarding persona (parallel to "Become an Artist", scoped to publishing samples only); and listing samples as a TuneCamp website use case.

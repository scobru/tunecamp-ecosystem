...
- **Transport & auth**: Federated instances relay over `POST /api/chat/federated/inbound`, authenticated with an `X-Chat-Signature` header over the JSON encoding of `[username, instance, text, ts, lobby, toUsername, roomGlobalId, roomName]`. The fields are JSON-encoded rather than joined on a separator so that a separator character inside the attacker-controlled `text` cannot produce the same signing input as a different message. A sender signs with its own site actor key — RSA-SHA256 under `site_private_key`, the same keypair its ActivityPub actor publishes — and falls back to HMAC-SHA256 with the shared `TUNECAMP_CHAT_FEDERATION_SECRET` only when it has no site key. The receiver resolves the claimed instance's public key through NodeInfo `metadata.actorId` → the actor's `publicKey.publicKeyPem`, and caches it in `remote_actors`. The endpoint fails closed with `503` when neither a shared secret nor a local `site_public_key` is configured, and returns `401` on a bad signature.
- **A resolved key is conclusive**: if the claimed peer publishes a key, the signature is checked against that key alone — a shared-secret signature is refused, because otherwise anyone holding the federation secret could downgrade their way into signing as that host simply by making the key check fail. The shared secret is accepted only from a peer that publishes no key at all. One operational consequence: if a peer's key cannot be fetched (its NodeInfo or actor endpoint is briefly unreachable) its first message is refused with `401`; once fetched the key is cached and reused, so this does not repeat.
...
- **Trust model — read this before deploying**: for a peer that publishes a site actor key, the signature pins a message to that one host. That is every peer running 5.1.0 or later, since the site keypair is generated automatically. The shared secret survives only for a peer that publishes no key, and there a valid signature still proves *some* peer sent the message, not *which* one: any holder can sign as any user of any keyless instance, bounded only by the known-peer check. Treat it as a federation-wide trust boundary, not a per-peer credential, and only share it with operators you trust.
- **Cross-instance chat requires peers on 5.1.0 or later**: an instance on an older release signs with the shared secret while already publishing a site actor key, so its messages are refused by an updated receiver. Upgrade both sides before relying on cross-instance chat.
...
- **`TUNECAMP_CHAT_FEDERATION_SECRET`** (env var): Legacy shared HMAC secret for cross-instance chat federation, used only where a peer publishes no site actor key — signing is otherwise the instance's own RSA site key. With neither this secret nor a local `site_public_key`, federated relay is disabled (`/inbound` returns `503`). Shared by every peer, so wherever it still applies it is a federation-wide trust boundary — see [Federated Chat](#federated-chat-cross-instance).

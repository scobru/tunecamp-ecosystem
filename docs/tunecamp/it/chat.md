...
- **Trasporto e autenticazione**: Le istanze federate si scambiano messaggi tramite `POST /api/chat/federated/inbound`, autenticato con un header `X-Chat-Signature` sulla codifica JSON di `[username, instance, text, ts, lobby, toUsername, roomGlobalId, roomName]`. I campi sono codificati in JSON invece che uniti da un separatore, così un carattere separatore dentro il campo `text` (controllato dall'attaccante) non può produrre lo stesso input di firma di un messaggio diverso. Il mittente firma con la chiave del proprio attore di sito — RSA-SHA256 con `site_private_key`, la stessa coppia che pubblica il suo attore ActivityPub — e ricade su HMAC-SHA256 con il segreto condiviso `TUNECAMP_CHAT_FEDERATION_SECRET` solo se non ha una chiave di sito. Il ricevente risolve la chiave pubblica dell'istanza dichiarata tramite NodeInfo `metadata.actorId` → `publicKey.publicKeyPem` dell'attore, e la mette in cache in `remote_actors`. L'endpoint fallisce in modo chiuso con `503` quando non sono configurati né un segreto condiviso né una `site_public_key` locale, e restituisce `401` su firma non valida.
- **Una chiave risolta è conclusiva**: se il peer dichiarato pubblica una chiave, la firma viene verificata solo contro quella chiave — una firma con segreto condiviso viene rifiutata, altrimenti chiunque possieda il segreto di federazione potrebbe declassare la verifica e firmare a nome di quell'host semplicemente facendo fallire il controllo della chiave. Il segreto condiviso è accettato solo da un peer che non pubblica alcuna chiave. Una conseguenza operativa: se la chiave di un peer non è recuperabile (NodeInfo o endpoint attore momentaneamente irraggiungibili) il suo primo messaggio viene rifiutato con `401`; una volta recuperata la chiave resta in cache e la cosa non si ripete.
...
- **Modello di fiducia — leggere prima di mettere in produzione**: per un peer che pubblica una chiave attore di sito, la firma lega il messaggio a quel singolo host. Vale per ogni peer dalla 5.1.0 in poi, dato che la coppia di sito viene generata automaticamente. Il segreto condiviso sopravvive solo per i peer che non pubblicano alcuna chiave, e lì una firma valida dimostra ancora che il messaggio arriva da *un* peer, non da *quale* peer: chiunque lo possieda può firmare a nome di qualsiasi utente di un'istanza senza chiave, con il solo limite del controllo peer conosciuto. Va trattato come confine di fiducia dell'intera federazione, non come credenziale per singolo peer, e condiviso solo con operatori fidati.
- **La chat cross-instance richiede peer dalla 5.1.0 in su**: un'istanza su una release precedente firma con il segreto condiviso pur pubblicando già una chiave attore di sito, quindi i suoi messaggi vengono rifiutati da un ricevente aggiornato. Aggiorna entrambi i lati prima di contare sulla chat cross-instance.
...
- **`TUNECAMP_CHAT_FEDERATION_SECRET`** (variabile d'ambiente): Segreto HMAC condiviso legacy per la federazione chat cross-instance, usato solo dove un peer non pubblica una chiave attore di sito — altrimenti si firma con la chiave RSA di sito dell'istanza. Senza né questo segreto né una `site_public_key` locale, il relay federato è disabilitato (`/inbound` risponde `503`). È condiviso da tutti i peer, quindi dove ancora si applica è un confine di fiducia dell'intera federazione — vedi [Chat Federata](#chat-federata-cross-instance).

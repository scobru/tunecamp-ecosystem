...
- [ ] `webapp/vite.config.ts:18-19` — `vite-plugin-node-polyfills` include `dgram`/`child_process`/`os`/`zlib`/`stream`, residuo dello stack ZEN/Gun.js rimosso. Fix: ridurre a quanto realmente necessario per `ethers`.
